...
public class Connections {
    ...
    public boolean getDBConnection() { // connect to DB
        if (connection == null) ;
        {
            try {
            	...
            	connection = DriverManager.getConnection("jdbc:sqlite:filmy.db"); //sem treba dat cestu kde sa uklada .db subor
                createTableHranyF();
    ...
    public void createTableHranyF() //vytvori tabulku hranych filmov
    {
    ...
    public boolean resetTableHranyF() //premaze tabulku hranych filmov
    {
        if (connection==null)
            return false;
    ...
    public void createTableAnimovanyF()// to iste co pri HranyF
    {
    ...
    public boolean resetTableAnimovanyF()//to iste co pri HranyF
    {
        if (connection==null)
            return false;
    ...
    public void getDB(Databaza Films) //nacita z sql databaze
    {

        this.createTableHranyF();
        this.createTableAnimovanyF();
        Films.vymazanieDatabazy();
        String sql = "SELECT * FROM hranyf;";  //nacitanie z tabulky hranych filmov
        try
        {
        ...
        sql = "SELECT * FROM animovanyf;"; // nacitanie z tabulky animovanych filmov
        try
        {
    ...
    public void saveDB(Databaza Films) //uklada do sql databaze
    {
        ...
        String sql = "INSERT INTO hranyf (nazov, reziser, rokVydania, zoznamHercov) VALUES (?,?,?,?)"; //ulozenie hranych filmov
        try
        {
            for (Film o: hranyF)
            {
                ...
                //pstmt.setArray(4, connection.createArrayOf("varchar", o.getHodnotenia().toArray()));
        ...
        String sqll = "INSERT INTO animovanyf (nazov, reziser, rokVydania,  zoznamAnimatorov) VALUES (?,?,?,?)"; //ulozenie studentov hum oboru
        try
        {
            for (Film o: animovanyF)
            {
                ...
                //pstmtt.setArray(4, connection.createArrayOf("varchar", o.getHodnotenia().toArray()));
